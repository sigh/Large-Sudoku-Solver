...
fn guess_dimension(num_cells: usize) -> Option<u32> {
  ...
  if dim < 2 || dim > 11 { return None; }
  if dim*dim*dim*dim != num_cells { return None; }
  Some(dim as u32)
}
...
fn parse_grid_layout(input: &str) -> Option<(Shape, FixedValues)> {
  ...
  let parts = CELL_REGEXP.find_iter(input).map(|mat| mat.as_str()).collect::<Vec<_>>();
  ...
  for (i, part) in parts.iter().enumerate() {
    match *part {
      ...
      _ => {
        ...
        if value <= 0 || value > num_values { return None; }
